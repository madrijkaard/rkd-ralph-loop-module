...
pub async fn find_all_by_project_id(pool: &PgPool, project_id: i32) -> Result<Vec<UseCase>, sqlx::Error> {
    sqlx::query_as::<_, UseCase>(
        "SELECT id, name, specification, created_date, last_modified_date, project_id
         FROM use_case
...
pub async fn insert(
    ...
    sqlx::query_as::<_, UseCaseCreateResponse>(
        "INSERT INTO use_case (name, specification, created_date, last_modified_date, status, project_id)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING id, name, specification, created_date, project_id",
    )
    .bind(name)
    .bind(specification)
    .bind(now)
...
pub async fn update(
    ...
    sqlx::query_as::<_, UseCase>(
        "UPDATE use_case
         ...
         RETURNING id, name, specification, created_date, last_modified_date, project_id",
    )
    .bind(name)
    .bind(specification)
    .bind(now)
