...
pub async fn find_all(pool: &PgPool) -> Result<Vec<Task>, sqlx::Error> {
    sqlx::query_as::<_, Task>("SELECT id, name, sequence, type, path, prompt, created_date, last_modified_date, use_case_id FROM task WHERE status = $1 ORDER BY id")
        .bind(Status::A)
        .fetch_all(pool)
        .await
}
...
pub async fn find_by_id(pool: &PgPool, id: i32) -> Result<Option<Task>, sqlx::Error> {
    sqlx::query_as::<_, Task>("SELECT id, name, sequence, type, path, prompt, created_date, last_modified_date, use_case_id FROM task WHERE id = $1 AND status = $2")
        .bind(id)
        .bind(Status::A)
        .fetch_optional(pool)
        .await
